...
template<typename _Col>
ContainerInterface* _ContainerInterfaceCast(void* pRawChild){
	if constexpr(std::is_base_of<ContainerInterface, _Col>::value == true)
		return (ContainerInterface*)(((_Col*)pRawChild));
	else
		return nullptr;
}